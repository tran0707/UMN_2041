...
let rec exec (s: stmt) (stt: state) : state =
  ...
  | IfThenElse (e,s1,s2) -> (match eval e stt with
                             | Bool false -> exec s2 stt 
                             | Bool true ->  exec s1 stt
                             | _ -> raise (Failure "Incompatible types on 
                                          IfThenElse")
                             )
  | Skip -> stt
  | For (st,lwbound, upbound,body) -> (match eval lwbound stt with
                                       | Int low -> 
                                                (match eval upbound stt with
                                                 | Int high -> 
                                                     if (low <= high) then
                       exec (For(st,Val(Int(low +1)), Val(Int(high)),body)) (exec body ((st ,Int low)::stt))
                                                     else
                                                     exec Skip stt
                                                  | _ -> raise (Failure "Incompatible types on
                                                                For")
